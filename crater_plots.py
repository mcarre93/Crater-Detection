...
def plot_confusion_matrix(cls_pred, data_test):
    ...
    # ACCESS CONFUSION MATRIX VALUES
    tp = cm[0][0]
    fp = cm[0][1]
    fn = cm[1][0]
    tn = cm[1][1]
    ...
    # CALCULATE F1 SCORE
    f1 = f1_score(y_true = cls_true,
                     y_pred = cls_pred)

    # CALCULATE MATTHEW'S CORRELATION COEFFICIENT
    mcc = matthews_corrcoef(y_true = cls_true, y_pred = cls_pred)
    ...
    plt.savefig('_ConfusionMatrix.png', bbox_inches='tight', dpi=1000)
    plt.matshow(cm)
...
def plot_conv_weights(weights, input_channel=0):
    ...
    # Ensure the plot is shown correctly with multiple plots
    # in a single Notebook cell.
    plt.savefig('_ConvulationLayerPlot.png', bbox_inches='tight', dpi=1000)
    plt.show()
...
def plot_conv_layer(values):
    ...
    # Ensure the plot is shown correctly with multiple plots
    # in a single Notebook cell.
    plt.savefig('_ActivationPlot.png', bbox_inches='tight', dpi=1000)
    plt.show()
