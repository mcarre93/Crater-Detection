...
# EDIT: Add functions to visualize CNN
# EDIT: Added self as parameter
    def getActivations(self, layer,stimuli,size=900):
        x = tf.placeholder(tf.float32, (None, size),name="x-in")
        keep_prob = tf.placeholder("float")
        units = self._session.run(layer,feed_dict={x:(np.asarray(stimuli, dtype = "float32"),(-1)),keep_prob:1.0})
        plotNNFilter(units)
    ...
    def plotNNFilter(units):
        ...
        for i in range(filters):
            ...
            plt.savefig('_NNPlot.png', bbox_inches='tight', dpi=1000)
